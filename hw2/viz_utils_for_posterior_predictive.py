...
def plot_potential_energy_of_samples(history_PE_by_chain,
    ...
    axL.set_xlabel('all iterations (including burnin), symlog scale')
    axL.set_ylabel('potential energy');
    axL.set_xscale('symlog', linthresh=25, base=2);
    axL.set_xlim([0, B+S])
    axL.set_xticks([0, 10, 20, 40, 100, 200, 400]);
    axL.set_xticklabels([0, 10, 20, 40, 100, 200, 400]);
    axL.legend(loc='upper right');

    axR = axgrid[1]
    for chain in chain_list:
        axR.plot(np.arange(B, B+S), history_PE_by_chain[chain][B:])
    axR.set_xlabel('kept samples (after burnin)')
