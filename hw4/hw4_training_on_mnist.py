...
if __name__ == "__main__":
    ...
    parser.add_argument(
        ...
        help='Learning rate for grad. descent (default: 0.01)')
    parser.add_argument(
    ...
    parser.add_argument(
        '--data_folder', type=str, default=os.path.abspath('.'),
        help='Folder where you have MNIST data stored')
    ...
    train_loader = torch.utils.data.DataLoader(
        datasets.MNIST(
            args.data_folder, train=True, download=True,
            transform=transforms.Compose([
                transforms.CenterCrop((S)),
                transforms.ToTensor(), torch.round])),    
        batch_size=args.batch_size,
        shuffle=True,
        drop_last=True) # avoid batches of different size!

    eval_batch_size = 20000
    train_eval_loader = torch.utils.data.DataLoader(
        datasets.MNIST(
            args.data_folder, train=True,
            transform=transforms.Compose([
                transforms.CenterCrop((S)),
                transforms.ToTensor(), torch.round])),    
    ...
    for epoch in range(args.n_epochs + 1):
        ...
        ## Only save results for epochs 0,1,2,3,4 and 10,20,30,...
        ...
        with torch.no_grad():
            ...
            # Could save model to disk if useful
